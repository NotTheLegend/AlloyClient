...
public class ClassInfo : Container {
    ...
    private void UpdateInfo(ClassRect classRect) {
        ...
        var startingItems = new[] {
            ObjectLibrary.GetItem(props.Equipment[0] ?? 0), ObjectLibrary.GetItem(props.Equipment[1] ?? 0),
            ObjectLibrary.GetItem(props.Equipment[2] ?? 0), ObjectLibrary.GetItem(props.Equipment[3] ?? 0),
        };
