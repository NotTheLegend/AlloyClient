# Alloy Client
A recreation of the [7.0 RotMG Client](https://github.com/moistosaurus/realm-cli) written in C# using OpenGL via OpenTk

# Alloy Server
The server this client connects to out of the box can be at the sister project over at [Zolmex/alloy-server](https://github.com/Zolmex/alloy-server)

# Progress
Very unfinished\
use at your own risk, i can/will push breaking changes at anytime

# Build Requirements
### Windows
* net 10
### Linux
* net 10
* wine (ContentBuilder -> Font Atlas)

# Credits
#### Main Developers
* [Link](https://github.com/NotTheLegend)
#### With help from
* [Zolmex](https://github.com/Zolmex)
* [HaloC](https://github.com/HaloCelsius)
#### With minor contributions from
* [Cuckson](https://github.com/Cuckson)
* [patpot](https://github.com/patpot)
* [Animus](https://github.com/animusDS)
* [NekoT](https://github.com/EtichBruh)
* runes
* [minuie](https://github.com/minuie)
#### Special thanks to
* [Slendergo](https://github.com/Slendergo/)
* akseli