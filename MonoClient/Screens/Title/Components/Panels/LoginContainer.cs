...
public class LoginContainer : Panel {
    ...
    public LoginContainer() {
        ...
        var registerConfig = new TextButtonConfig { Text = "New user? Click here to Register!", FontSize = 16, OnClicked = () => { State = PanelState.Closed; PanelManager.Enqueue(new RegisterContainer()); }, Bold = true, X = Width / 2, Y = _passwordInput.Y + 40, Anchor = UiAnchor.Middle };
        var registerButton = new TextButton(registerConfig);
        AddChild(registerButton);
        
        var forgotConfig = new TextButtonConfig { Text = "Forgot Password?", FontSize = 16, OnClicked = () => { State = PanelState.Closed; PanelManager.Enqueue(new ForgotContainer()); }, Bold = true, X = Width / 2, Y = registerButton.Y + 30, Anchor = UiAnchor.Middle };
