...
public class LoginContainer : Overlay {
    ...
    public LoginContainer() {
        ...
        var title = new SimpleText(new TextConfig { Text = "Log in", FontSize = 22, Bold = 1, X = Width / 2, Y = titleBackground.Height / 2, Color = 0xFFFFFF, Anchor = UiAnchor.Middle });
        AddChild(title);

        var emailConfig = new InputConfig { X = Width / 2, Y = 100, FontSize = 24, Bold = 1, Color = 0xFFFFFF, Width = 350, DefaultText = "Email", Anchor = UiAnchor.Middle };
        _emailInput = new TextInput(emailConfig);
        AddChild(_emailInput);

        var passwordConfig = new InputConfig { X = Width / 2, Y = 160, FontSize = 24, Bold = 1, Color = 0xFFFFFF, Width = 350, DefaultText = "Password", Password = true, Anchor = UiAnchor.Middle };
        _passwordInput = new TextInput(passwordConfig);
        AddChild(_passwordInput);

        var registerConfig = new TextButtonConfig { Text = "New user? Click here to Register!", FontSize = 16, OnClicked = () => { CloseOverlay(); OverlayManager.Enqueue(new RegisterContainer()); }, Bold = 1, X = Width / 2, Y = _passwordInput.Y + 40, Anchor = UiAnchor.Middle };
        var registerButton = new TextButton(registerConfig);
        AddChild(registerButton);
        
        var forgotConfig = new TextButtonConfig { Text = "Forgot Password?", FontSize = 16, OnClicked = () => { CloseOverlay(); OverlayManager.Enqueue(new ForgotContainer()); }, Bold = 1, X = Width / 2, Y = registerButton.Y + 30, Anchor = UiAnchor.Middle };
        var forgotButton = new TextButton(forgotConfig);
        AddChild(forgotButton);
        
        var loginConfig = new TextButtonConfig { Text = "Log in", FontSize = 28, OnClicked = OnLogin, Bold = 0, X = Width - 25, Y = Height - 25, Anchor = UiAnchor.RightBottom };
