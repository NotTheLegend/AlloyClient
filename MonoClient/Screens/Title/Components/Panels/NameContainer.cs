...
public class NameContainer : Overlay {
    ...
    public NameContainer() {
        ...
        var title = new SimpleText(new TextConfig { Text = "Set Name", FontSize = 22, FontType = FontType.Bold, X = Width / 2, Y = titleBackground.Height / 2, Color = 0xFFFFFF, Anchor = UiAnchor.Middle });
        AddChild(title);
        
        var nameConfig = new InputConfig { X = Width / 2, Y = 100, FontSize = 24, FontType = FontType.Bold, Color = 0xFFFFFF, Width = 350, DefaultText = "Name", Anchor = UiAnchor.Middle };
        _nameInput = new TextInput(nameConfig);
        AddChild(_nameInput);
        
        var loginConfig = new TextButtonConfig { Text = "Name", FontSize = 28, OnClicked = OnSetName, FontType = FontType.Normal, X = 475 - 25, Y = Height - 25, Anchor = UiAnchor.RightBottom };
