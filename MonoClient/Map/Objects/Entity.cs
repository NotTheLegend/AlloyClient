        ...
        foreach (var stat in statData) {
            switch (stat.Type) {
                ...
                case StatsType.ItemData19:
                    index = stat.Type - StatsType.ItemData0;
                    if (stat.Text != "{}") {
                        Equipment[index]?.Import(JsonConvert.DeserializeObject<IDictionary<string, object>>(stat.Text));
                    }
                    break;
                case StatsType.Effects2:
                    ConditionEffectBatches[ConditionEffectUtil.SecondBatch] = (uint)stat.Value;
                    break;
                case StatsType.PlayerHost:
                    PlayerHost = stat.Value;
                    break;
                case StatsType.PermaPet:
                    PermaPet = stat.Value == 1;
                    break;
                case StatsType.Effects3:
                    ConditionEffectBatches[ConditionEffectUtil.ThirdBatch] = (uint)stat.Value;
                    break;
                case StatsType.MadnessPullRadius:
                    MadnessPullRadius = stat.Value;
                    break;
                case StatsType.MadnessPullMaxSpeed:
                    MadnessPullMaxSpeed = stat.Value;
                    // if (!Map.MadnessEntitiesDict.ContainsKey(ObjectId)) {
                    //     Map.MadnessEntitiesDict[ObjectId] = this;
                    // }
                    break;
                case StatsType.QuestGlowColor:
                    QuestGlowColor = stat.Value;
                    break;
                case StatsType.MinimumHp:
                    MinimumHp = stat.Value;
                    break;
                case StatsType.DontFaceAttacks:
                    DontFaceAttacks = stat.Value == 1;
                    break;
                case StatsType.AllDebuffsImmune:
                    AllDebuffsImmune = stat.Value == 1;
                    break;
                case StatsType.ThresholdData:
                    ThresholdData = (ThresholdData)stat.DataObject;
                    break;
                case StatsType.StackEffects:
                    StackEffectData = (StackEffectData)stat.DataObject;
                    break;
                case StatsType.CondDurations:
                    ConditionDurationData = (ConditionDurationData)stat.DataObject;
                    break;
                case StatsType.DungeonModifiers:
                    DungeonModifiersStats = (DungeonModifiersStat)stat.DataObject;
                    break;
                case StatsType.DamagersCount:
                    DamagersCount = stat.Value;
                    break;
                case StatsType.CustomTexture:
                    CustomTexture = stat.Value;
                    break;
            }
        }
    }
